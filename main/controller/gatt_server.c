...
#define TEST_DEVICE_NAME            "TO_THE_MOON"
...
static _Bool is_notify_enabled = 0;
...
void gatt_server_send_notify(uint8_t *data, uint16_t len) {
    struct gatts_profile_inst *params = &gl_profile_tab[PROFILE_A_APP_ID];
    esp_ble_gatts_send_indicate(params->gatts_if, params->conn_id, params->char_handle,
                                len, data,
                                false);
}
            ...
            if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
                ...
                if (descr_value == 0x0001){
                    if (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY){
                        ...
                        /* START CUSTOM CODE */
                        is_notify_enabled = 1;
                        uint32_t notify_data = proximity_get_counter();
